...
mints AS (
    ...
    FROM (
        ...
        UNION ALL
        SELECT 'bnb' AS chain, evt_block_time, evt_block_number, minter, contract_address, evt_tx_hash, mintAmount, mintTokens FROM venus_bnb.vbep20_bnb_core_evt_mint
        UNION ALL
...
LEFT JOIN prices.usd p ON t.chain = p.blockchain AND DATE_TRUNC('minute', t.evt_block_time) = p.minute AND m.underlying_token_address = p.contract_address
